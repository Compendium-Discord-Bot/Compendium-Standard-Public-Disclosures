...
## II. What do we use this data for?

- To deliver Compendium's service of writing management for yourself and for servers, and knowledge management, within servers.
...
Everything we store, we store because we need it to run the service - not because we can.

- **Your Discord data and documents**: We need this to actually provide Compendium. Without it, there is no service. *(GDPR: contract performance, Art. 6(1)(b))*
...
- **Document data**: Stored until the document owner requests deletion. Two types exist:
  - **Server-owned documents** (guild-wide entries like locations, NPCs, and lore): if the bot is removed from the server or the server is deleted, these will be transferred to the user designated in discord server settings as owner. *(This is planned behaviour - it will be available when document management launches.)*
...
- **Infraction evidence**: Evidence is handled in stages:
  ...
  - **After the appeal period closes**: detailed evidence is deleted. Where we have a documented legitimate interest - such as establishing a pattern of behavior or defending against a legal claim - a staff-written description of the infraction may be retained in its place. This description characterises what occurred without retaining the original personal data.
  - **Pattern-of-behavior retention**: if a subsequent related infraction occurs, the description from the prior infraction may be retained for `[WIP - base retention period]` from the date of the later infraction, resetting each time a related infraction is found. Staff must document the connection at the time of the later infraction - it cannot be determined retroactively.
  ...
  - **Illegal material** (e.g. CSAM): the user is banned immediately. We comply with all applicable mandatory reporting obligations and report the material to the appropriate authorities. We retain only a written description — the original material is not kept beyond what is needed to make the report. We await guidance from authorities on whether they wish to pursue further investigation.
...
- **Support request data**: Retained for 6 months after the resolution of the request, unless required for longer to fulfil legal obligations, or when it is related to or results in an infraction, in which case it follows the infraction retention rules above.
- **Translation contributions**: Retained for as long as the contribution is active and in use. Deleted when superseded by an updated contribution or upon the contributor's request.
- **Discord metadata** (guild IDs, role IDs, thread IDs, message IDs): Retained for as long as the associated documents or server relationship exists. Removed when all associated documents are deleted and the bot no longer has an active relationship with the server.
...
- **Rectification / Correct**: If any data we have is inaccurate, you have the right to request corrections. *(GDPR Art. 16 / CPRA)*
...
- **Object**: If we're processing your data based on our interests rather than a contract (e.g., infraction records), you can ask us to stop. We'll stop unless we have a compelling reason not to. *(GDPR Art. 21)*
- **Restrict**: If you dispute whether our data about you is accurate, you can ask us to freeze processing until we sort it out. *(GDPR Art. 18)*
- **Complain**: If you're in the EU and feel we're handling your data improperly, you have the right to file a complaint with your country's data protection authority. We'd always prefer you contact us first so we can fix it. *(GDPR Art. 77)*

To exercise any of these rights, please open a request via `[WIP - support channel]` or contact us at `[WIP - contact email]`.

### 1. Limitations

Not every data right applies in every circumstance. These limitations exist not to disadvantage you, but because some data serves purposes that would be undermined if it could be freely modified or deleted on request. Infraction records, for example, exist to protect all users of the service — not just the person they relate to. Allowing a user to erase or alter that record would compromise our ability to keep the service safe for everyone else.

Every limitation listed here has a specific, documented reason. None of them are blanket overrides of your rights - they apply only to the data categories and circumstances described.

#### 1.1. Infraction Data

1. Reporter and infraction data is confidential and will not be shared.
2. The infraction summary record (what happened, when, appeal outcome) is retained indefinitely and will not be deleted except as required by applicable law. Evidence associated with an infraction follows retention rules in §III.
3. Any documents, materials, or information relevant to an active infraction review will not be deleted until that review is complete.
...
- Discord - handles authentication and is the platform through which you interact with the bot

We only engage providers who offer data protection standards consistent with GDPR. Any provider that processes your data on our behalf is required to do so under a data processing agreement — they may not use your data for their own purposes. If we change providers in a way that would affect the data protection regime applicable to your data — for example, switching to a provider outside the EU — we will treat that change with the same seriousness as moving our own infrastructure. We will give you advance notice, explain why the change is necessary, and ensure appropriate legal safeguards are in place (such as Standard Contractual Clauses). This is not something we would do for convenience.

### 1. Where Your Data Lives

We plan to host Compendium's servers in Europe. This means your data stays within the EU, subject to EU data protection law (GDPR).

If this changes - for example, if we move to a different region - we will update this section and notify users in advance. We want to be honest with you: moving data outside the EU is not something we would do lightly. Different countries offer different levels of data protection, and some offer less than the EU does. A change of this kind would be a significant inconvenience to you as a user, and we acknowledge that directly. We would only consider it out of genuine operational necessity - not convenience or cost - and we would give you advance notice, explain why, and tell you what protections would apply in the new location.

## VI. Data Security

We take data security seriously. The following measures are either already in place or will be in place before the service launches:
...
We will ***`NEVER`*** sell your data. Not to advertisers, not to data brokers, not to anyone. Our users' data is not a product.
Do not approach us with any proposition to buy user data - you will be wasting your time, and ours.

## VIII. Data Sharing

We do not voluntarily share your data with any third party. The only circumstance under which we share data is when compelled by a legally binding order, under the strict conditions below.
...
- **Canary statement**: a quarterly-updated declaration covering whether we have ever received demands accompanied by gag orders - requests we are legally prohibited from even acknowledging. If the statement is removed or goes stale, that is the signal.
...
### 1. What we will not be deleting as part of data requests
...
- If a backup is restored before it expires, all logged deletion requests are re-applied before the data is put back into use.
- Backups that are never restored will naturally expire within 1 year, at which point the data is gone.
...
## XII. If You're in California

California law (CCPA/CPRA) gives you some specific rights on top of what we already offer everyone:
...
- We believe that a privacy policy shouldn't be used to justify stealing or selling user data.
